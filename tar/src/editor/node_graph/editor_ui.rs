...
use crate::editor::node_graph::color_hex_utils::*;
use crate::editor::node_graph::utils::ColorUtils;
    ...
    CategoryType: CategoryTrait,
{
    ...
    fn draw_grid_background(&self, rect: Rect, painter: &Painter) {
        // Background color (dark blue-gray like Unreal)
        let bg_color = Color32::from_rgb(38, 38, 38);
        painter.rect_filled(rect, 0.0, bg_color);

        let zoom = self.pan_zoom.zoom;
        let pan_offset = self.pan_zoom.pan;

        // Grid settings - scale with zoom
        let base_small_grid = 15.0;
        let small_grid_size = base_small_grid * zoom;
        let large_grid_size = small_grid_size * 10.0;

        // Fade out small grid when zoomed out
        let small_alpha = ((zoom - 0.3) * 40.0).clamp(0.0, 15.0) as u8;
        let large_alpha = 40u8;

        let small_grid_color = Color32::from_rgba_unmultiplied(255, 255, 255, small_alpha);
        let large_grid_color = Color32::from_rgba_unmultiplied(255, 255, 255, large_alpha);

        // Convert pan_offset to screen space offset
        let center = rect.center().to_vec2();
        let grid_origin = (pan_offset + rect.min.to_vec2()) * zoom;
        let screen_offset = grid_origin + center * (1.0 - zoom);

        // Draw small grid (skip if too zoomed out)
        if small_alpha > 0 {
            let start_x = ((rect.left() - screen_offset.x) / small_grid_size).floor()
                * small_grid_size
                + screen_offset.x;
            let start_y = ((rect.top() - screen_offset.y) / small_grid_size).floor()
                * small_grid_size
                + screen_offset.y;
            ...
            while x <= rect.right() {
                if x >= rect.left() {
                    painter.line_segment(
                        ...
                        egui::Stroke::new(1.0, small_grid_color),
                    );
                }
                x += small_grid_size;
            }
        ...
        // Draw large grid
        let start_x_large = ((rect.left() - screen_offset.x) / large_grid_size).floor()
            * large_grid_size
            + screen_offset.x;
        let start_y_large = ((rect.top() - screen_offset.y) / large_grid_size).floor()
            * large_grid_size
            + screen_offset.y;

        let mut x = start_x_large;
        while x <= rect.right() {
            if x >= rect.left() {
                painter.line_segment(
                    [egui::pos2(x, rect.top()), egui::pos2(x, rect.bottom())],
                    egui::Stroke::new(1.0, large_grid_color),
                );
            }
            x += large_grid_size;
        }

        let mut y = start_y_large;
        while y <= rect.bottom() {
            if y >= rect.top() {
                painter.line_segment(
                    [egui::pos2(rect.left(), y), egui::pos2(rect.right(), y)],
    ...
    fn draw_graph_editor_inside_zoom(
        ...
        self.draw_grid_background(editor_rect, &painter);
