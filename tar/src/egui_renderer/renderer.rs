        ...
        let pipeline = {
            device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
                ...
                fragment: Some(wgpu::FragmentState {
                    module: &module,
                    entry_point: Some(if output_color_format.is_srgb() || output_color_format == wgpu::TextureFormat::Rgba16Float || output_color_format == wgpu::TextureFormat::Rgba32Float {
                        eprintln!("Detected a linear (sRGBA aware) framebuffer {:?}. egui prefers Rgba8Unorm or Bgra8Unorm", output_color_format);
                ...
                match &clipped_primitive.primitive {
                    ...
                    Primitive::Callback(callback) => {
                        if let Some(c) = callback.callback.downcast_ref::<Callback>() {
                            ...
                            eprintln!("Unknown paint callback: expected `egui_wgpu::Callback`");
                        };
                        acc
                    }
                }
